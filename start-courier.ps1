...
foreach ($courierPort in @(8000, 3000)) {
...
try {
    ...
    $courierFrontend = Start-Process -FilePath $courierNode -ArgumentList @(('"' + $courierNext + '"'), 'start', '--hostname', '127.0.0.1') -WorkingDirectory (Join-Path $courierRoot 'frontend') -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $courierLogDir 'frontend.stdout.log') -RedirectStandardError (Join-Path $courierLogDir 'frontend.stderr.log')
    Write-Host 'CourierAI: http://localhost:3000'
